...
public class baekjoon16173 {
    ...
    static void bfs(int[][] game, boolean[][] visited) {
        ...
        while(!q.isEmpty()) {
            ...
            int bottom = row + game[row][col];
            int right = col + game[row][col];

            if(bottom < game.length && !visited[bottom][col]) {
                q.add(new int[] {row + game[row][col], col});
                visited[bottom][col] = true;
            }

            if(right < game.length && !visited[row][right]) {
                q.add(new int[] {row, col+game[row][col]});
